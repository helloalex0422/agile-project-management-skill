...
读取 `~/.claude/agile-pm/standup-state.json`：

```bash
cat ~/.claude/agile-pm/standup-state.json
```
...
```
✅ 今日站会已于早些时候完成，无需重复进行。
如需重新开始，请删除或更新 ~/.claude/agile-pm/standup-state.json 中的 last_done 字段。
```
...
参照 `adapters/notion.md` 中的 `list_tasks` 实现，使用 notion-tool 的 `query-database` 命令读取以下任务：
- Status = "In Progress"
- Status = "Sprint Todo"（当前 Sprint）

同时读取 `~/.claude/agile-pm/standup-state.json` 中的 `last_snapshot`（上次站会快照）。若文件不存在，视为首次运行，`last_snapshot` 为空。
...
根据用户选择，参照 `adapters/notion.md` 中的 `update_task` / `move_task` 实现，使用 notion-tool 的 `update-page` 命令更新对应任务：
- 选 1：更新 Status = "Done"
- 选 2：若用户有补充说明，追加到 Notes（原内容 + "\n" + 今日日期 + ": " + 说明）
- 选 3：更新 Priority = "P0"，追加到 Notes（原内容 + "\n" + 今日日期 + ": [blocker] " + 原因）
- 选 4：不调用任何工具，直接跳过此任务，继续处理下一个
...
对用户选中的任务，参照 `adapters/notion.md` 中的 `move_task` 实现，更新 Status = "In Progress"。
...
1. 参照 `adapters/notion.md` 的 `list_tasks` 实现，查询当前 Sprint 所有任务，统计各状态数量
2. 参照 `adapters/notion.md` 的 `get_dashboard` 实现，从 Dashboard Page 读取现有 `recent_standups`（最多保留最近3条）
3. 将今日摘要插入最前，超出3条则丢弃最旧的
4. 参照 `adapters/notion.md` 的 `update_dashboard` 实现：删除旧 blocks，append 新内容
...
将当前状态写入 `~/.claude/agile-pm/standup-state.json`：
